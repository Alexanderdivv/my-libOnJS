...
export default class MathFighterScene extends Phaser.Scene {
  ...
  init() {
    ...
    // defaultnya dibuku itu 0 (number), ada error kalau kita inisialisaiin jadi tipe lain
    // solusinya itu untuk ngatasin error sebenarnya jadiin undefined aja
  ...
  create() {
    ...
    // challenge: create overlaps between player and slash
    ...
    // challenge, create overlaps between enemu and slash
    this.physics.add.overlap(
      this.slash,
      this.enemy,
      this.spriteHit,
  ...
  spriteHit(slash, sprite) {
    ...
    if (sprite.texture.key == "player") {
      ...
      // error ini liat di preload penjelasaan sama solusinya, atau bisa di ignore aja
